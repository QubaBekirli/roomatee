    ...
    <AppShell>
      <TopBar back title="Elan" right={
        <div className="flex gap-1">
          <button className="p-2 rounded-xl bg-secondary press-scale"><Share2 size={16}/></button>
          <button className="p-2 rounded-xl bg-secondary press-scale"><Heart size={16}/></button>
        </div>
      }/>
      <div className="relative aspect-[4/3] bg-muted">
        ...
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
          {l.images.map((_, i) => (
            <button key={i} onClick={() => setImg(i)} className={`w-1.5 h-1.5 rounded-full transition-all ${i === img ? "bg-white w-6" : "bg-white/50"}`}/>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pt-4 animate-fade-in">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold brand-gradient-text">₼{l.price}</div>
            <div className="text-[10px] text-muted-foreground">/aylıq</div>
          </div>
        </div>

        <div className="flex flex-wrap gap-2 mt-3">
          {l.verified && <Badge icon={<ShieldCheck size={12}/>} text="Verified Property" tone="primary"/>}
        ...
        {/* Owner */}
        <div className="mt-4 p-4 rounded-2xl bg-card border border-border flex items-center gap-3">
          ...
          <Link to="/messages/$id" params={{id:"t1"}} className="p-2.5 rounded-xl brand-gradient text-white press-scale"><MessageCircle size={16}/></Link>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {l.utilities.map(u => <span key={u} className="text-xs px-3 py-1.5 rounded-full bg-secondary flex items-center gap-1"><Wifi size={12}/>{u}</span>)}
        </div>

        <h3 className="font-bold mt-5 mb-2">Yaxınlıqda (Smart Location)</h3>
        <div className="grid grid-cols-2 gap-2">
          {l.nearby.map(n => (
            <div key={n.name} className="bg-card border border-border rounded-xl p-3 flex items-center gap-2">
      ...
      <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-md px-4 z-20">
        <button onClick={() => { setShowReserve(true); setStep(0); }} className="w-full py-3.5 rounded-2xl brand-gradient text-white font-bold shadow-xl shadow-primary/30 press-scale flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end max-w-md mx-auto animate-fade-in" onClick={() => setShowReserve(false)}>
          <div className="bg-card w-full rounded-t-3xl p-5 animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${i <= step ? "bg-primary/5 border-primary/30" : "bg-secondary border-border opacity-60"}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i <= step ? "brand-gradient text-white" : "bg-muted text-muted-foreground"}`}>{i+1}</div>
            ...
            <button
              ...
              className="w-full mt-5 py-3 rounded-xl brand-gradient text-white font-bold press-scale"
            >
